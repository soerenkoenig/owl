...
namespace owl
{
    ...
    && !is_array<Container>::value>>
  {
    ...
    static bool read(std::istream& in, value_type& value)
    {
      ...
      std::int64_t count;
      if (!owl::read_binary(in, count)) return false;
    ...
    {
      std::int64_t count = static_cast<std::int64_t>(value.size());
      if (!owl::write_binary(out, count)) return false;
  ...
  struct binary_serialization<std::vector<bool>, void>
  {
    ...
    static bool read(std::istream& in, std::vector<bool>& value)
    {
      ...
      {
        bool e;
        if (!owl::read_binary(in, e)) return false;
        elem = e;
      }
      return true;
    }
...
/*
 ...
 //           {o,o}
 //          ./)_)
 //      owl --"-"--- obviously well-made library
 //
 //  Copyright © 2017 Sören König. All rights reserved.
 //
 ...
 constexpr static bool is_serializable =
 binary_serialization<T1>::is_serializable &&
 binary_serialization<T2>::is_serializable;
 
 static bool read(std::istream& in, value_type& value)
 {
 if (!read_binary(in, value.first) || !read_binary(in, value.second)) return false;
 ...
 constexpr static bool is_serializable =
 (... && binary_serialization<std::decay_t<Args>>::is_serializable);
 ...
 template <typename Container>
 struct binary_serialization<Container, std::enable_if_t<is_sequence_container<Container>::value && !is_array<Container>::value>>
 {
 using value_type = Container;
 using elem_type =
 std::decay_t<decltype(*std::begin(std::declval<Container>()))>;
